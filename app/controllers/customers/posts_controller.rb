class Customers::PostsController < ApplicationController
  ...
  def index
    @posts = Post.joins(:customer).where(customers: { is_deleted: false }).sort_index(params[:sort]).page(params[:page])
  ...
  def post_params
    params.require(:post).permit(:title, :content, :map_latitude, :map_longitude, :departure_date, :prefecture, :return_date, images: [], location:[])
  end
